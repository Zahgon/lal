// Copyright 2020, Chef.  All rights reserved.
// https://github.com/q191201771/lal
//
// Use of this source code is governed by a MIT-style license
// that can be found in the License file.
//
// Author: Chef (191201771@qq.com)

package rtprtcp

// 通过收到的rtp包和rtcp sr包，产生rtcp rr包

type RrProducer struct {
	senderSsrc uint32
	mediaSsrc  uint32

	clockRate int

	maxSeq      int32
	baseSeq     int32
	cycles      uint32
	received    uint32
	extendedSeq uint32

	transit int64
	jitter  uint32

	expectedPrior uint32
	receivedPrior uint32
}

func NewRrProducer(clockRate int) *RrProducer { _ = "STUB: not implemented"; return nil }

// FeedRtpPacket 每次收到rtp包，都将seq序号传入这个函数
func (r *RrProducer) FeedRtpPacket(seq uint16) { _ = "STUB: not implemented"; return }

// Produce 收到sr包时，产生rr包
//
// @param lsr: 从sr包中获取，见func SR.GetMiddleNtp
// @return:    rr包的二进制数据
func (r *RrProducer) Produce(lsr uint32) []byte { _ = "STUB: not implemented"; return nil }

// @param timestamp 当前收到的rtp包头中的时间戳
func (r *RrProducer) updateJitter(timestamp uint32) {
	_ = "STUB: not implemented"
	// rfc3550 6.4.1 SR: Sender Report RTCP Packet
	// rfc3550 A.8 Estimating the Interarrival Jitter
	return
}

// 当前收到rtp包的本地物理时间

// 物理时间和包时间的差值，都换算成包时间戳格式

// 第一次跳过

// 这次差值，和上一次差值相减

// 一种设置jitter的方式
// set: r.jitter += (float32(1)/16) * (d - r.jitter)
// get: return r.jitter
//
// 另外一种方式
// 对应的get: return r.jitter >> 4
// 注意，右边的计算结果肯定是正数

func (r *RrProducer) getJitter() uint32 { _ = "STUB: not implemented"; return 0 }
